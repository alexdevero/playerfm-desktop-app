...
if (platform == 'darwin') {
  trayIcon = path.join(__dirname, 'assets/playerfm-logo.png')
} else if (platform == 'win32') {
  trayIcon = path.join(__dirname, 'assets/playerfm-logo.ico')
} else {
  trayIcon = path.join(__dirname, 'assets/playerfm-logo.ico')
}
...
app.once('ready', () => {
  ...
  const url = 'https://player.fm/' // process.argv[2]
  ...
  const menuTemplate = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Learn More',
          click: () => {require('electron').shell.openExternal('https://github.com/alexdevero/playerfm-desktop-app')}
        },
        {
          label: 'Author',
  ...
  appIcon.setTitle('PlayerFM')

  // Set toot tip for tray icon
  appIcon.setToolTip('PlayerFM')
